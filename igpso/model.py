...
class FeatureSelectionMLP(nn.Module):
    ...
    def reset_selection_rate(self):
        self.selection_rate = nn.Parameter(
            torch.ones([1, self.n_features])
        )
...
class Model(nn.Module):
    ...
    def reset_selection_rate(self):
        self.feature_selection_MLP.reset_selection_rate()
...
def reset_rate(model):
    layer = model.feature_selection_MLP  # 假设index是你想要清零的层的索引
    with torch.no_grad():  # 确保在权重更新时不计算梯度
